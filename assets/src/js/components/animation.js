        ...
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                ...
                const interval = setInterval(() => {
                    if ((endNumber > 0 && currentNumber >= endNumber) || (endNumber < 0 && currentNumber <= endNumber)) {
                        clearInterval(interval);
                        number.textContent = endNumber.toLocaleString();
                    } else {
                        currentNumber += increment;
                        number.textContent = (endNumber % 1 === 0 ? Math.round(currentNumber) : currentNumber.toFixed(1)).toLocaleString();
                    }
                }, 10);
