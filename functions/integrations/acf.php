...
add_action('init', function() {
    if( function_exists('acf_add_options_page') ) {
        ...
        //1.1.1 Menus (sibling to Theme Options)
        acf_add_options_page(array(
            'page_title' 	=> 'Menus',
            'menu_title' 	=> 'Menus',
            'menu_slug' 	=> 'acf-options-menus',
            'redirect' 		=> false
        ));
...
function mingo_acf_init_block_types() {
    ...
    if( function_exists('acf_register_block_type') ) {
        ...
        acf_register_block_type(array(
            ...
            'example'  => array(
				'attributes' => array(
					'mode' => 'preview',
					'data' => array(
						'preview_image_help' => get_template_directory_uri().'/assets/images/screenshots/block-hero.png',
					)
				)
			)
        ));
        ...
        acf_register_block_type(array(
            ...
            'title'             => __('Flexible Content', 'atlas'),
            'description'       => __('Add a flexible content block.', 'atlas'),
            'render_template'   => 'template-parts/blocks/block-flexible-content.php',
            'category'          => 'custom',
            'icon'              => $svg,
            'keywords'          => array( 'custom', 'flexible-content'),
            'mode'              => 'edit',
            'supports'          => array( 
                'align' 	=> false,
                'mode' 		=> true,
			...
			'example'  => array(
				'attributes' => array(
					'mode' => 'preview',
					'data' => array(
						'preview_image_help' => get_template_directory_uri().'/assets/images/screenshots/block-flexible-content.png',
					)
				)
			)
        ));
        ...
        acf_register_block_type(array(
			...
			'example'  => array(
				'attributes' => array(
					'mode' => 'preview',
					'data' => array(
						'preview_image_help' => get_template_directory_uri().'/assets/images/screenshots/block-heading.png',
					)
				)
			)
        ));
